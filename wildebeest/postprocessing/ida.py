...
def get_binja_folder_for_run(run:Run) -> str:
    return f'/run{run.number}.{run.config.name}.{run.build.recipe.name}'


#since no repo system in binja, we don't need to pass in the repo?
...
def do_import_binary_to_binja(run:Run, params:Dict[str,Any], outputs:Dict[str,Any]):
    ...
    for bid, fb in outputs['flatten_binaries'].items():
        ...
        ghidra_folder = get_binja_folder_for_run(run)
...
def binja_import(debug:bool, postscript:Path=None,
    get_postscriptargs:Callable[[FlatLayoutBinary], List[str]]=None,
    binja_path:str='', prescript:Path=None) -> RunStep:
    '''
    ...
    return RunStep(f'binja_import_{"debug" if debug else "strip"}', do_import_binary_to_binja, params)
